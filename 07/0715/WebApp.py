...
def translate_keyword(keyword, target_language='en'):
...
def search():
    ...
    if form.validate_on_submit():
        ...
        # 검색 키워드를 영어로 번역
        translated_keyword = translate_keyword(search_keyword)
        ...
        try:
            ...
            if filter_type == 'patent':
                ...
                if paper_conditions:
                    ...
                    filtered_df_papers = filtered_df_papers[(filtered_df_papers['title'].str.contains(translated_keyword, na=False)) | 
            ...
            return render_template('Web.html', table=Markup(table_html), 
...
def plot():
    ...
    if filtered_data_patents.empty:
        print("No data available for plotting")  # 디버깅 로그 추가
        return "No data available for plotting"

    if 'Application Date' not in filtered_data_patents.columns:
        print("Column 'Application Date' not found in filtered_data")  # 디버깅 로그 추가
